...
- Включена обработка клавиш:
    - отключает dev_hdd\boot*.txt из загрузки (бывает необходимо при конфликте с плагинами)
    - отключает все плагины в том числе и плагины hen, поэтому информацию о включении хен видно не будет, иконка не сменится, но ядро загрузится, в чем можно убедиться запустив игру/софт, либо обновив XMB (например через вход в дистанционное управление и выход - тогда иконка изменится)

...
## Изменения в 3.2.4 (тестовый)
- Новый способ установки HEN (для исключения проблем с разделом сеть)
...
- Добавлены пункты отключения syscall и включения их после отключения, проверки состояния
- Добавлен пункт перегрузки только LV2
- Добавлен пункты в раздел дамп инструментов (скорость вентиляторов, температура и другое)
- Добавлена возможность поменять кнопки Х и О местами в инструментах xRegistry
- Добавлен пункт установки/удаления плагина для отображения дополнительной информации в играх/хмб (фпс, температура и другое). Для настройки плагина нажмите L1+L3
- Добавлен пункт для управления трофеями (удаление всего набора трофеев, разблокировка/блокировка трофеев). **ВНИМАНИЕ синхронизация с PSN не работает.**

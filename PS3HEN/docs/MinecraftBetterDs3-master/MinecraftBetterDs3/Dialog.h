...
namespace Dialog
{
	...
	void DisplayMessage(const char* msg, int seconds = 5)
	{
		unsigned int type =
			CELL_MSGDIALOG_TYPE_BUTTON_TYPE_OK | CELL_MSGDIALOG_TYPE_DISABLE_CANCEL_ON | CELL_MSGDIALOG_TYPE_SE_TYPE_NORMAL;
		cellMsgDialogOpen2(type, msg, NULL, NULL, NULL);
		cellMsgDialogClose(seconds * 1000);
	}
	void Display(const char* msg, unsigned int type = CELL_MSGDIALOG_TYPE_BUTTON_TYPE_OK | CELL_MSGDIALOG_TYPE_SE_TYPE_NORMAL)
	{
		cellMsgDialogOpen2(type, msg, NULL, NULL, NULL);
	}
	void SetProgressValue(int value)
	{
		cellMsgDialogProgressBarInc(CELL_MSGDIALOG_PROGRESSBAR_INDEX_SINGLE, value);
	}

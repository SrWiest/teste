...
static MapEntry_t* getClosestMapping(const char *opath,uint32_t flags);
...
static void printMapTableList() {
		DPRINTF("printMapTableList: Mapping Items Count 0x%X / Mapping Table Size 0x%lX bytes : \n", getMapTableLength(),getMapTableByteSize());
		uint32_t idx=0;
		for(MapEntry_t* ptr = head; ptr != NULL;idx++,ptr = ptr->next) {
			//DPRINTF("mapping index %u: %s len: 0x%X byte(s) to %s len: 0x%X byte(s) with flags: %X\n", idx, ptr->oldpath, (uint32_t)ptr->oldpath_len, ptr->newpath, (uint32_t)ptr->newpath_len, ptr->flags);
			DPRINTF("mapping index %u:\n", idx);
			DPRINTF("flags: %X\n", ptr->flags);
			if(ptr->oldpath){
				DPRINTF("FROM: %s (len: 0x%X byte(s))\n", ptr->oldpath, (uint32_t)ptr->oldpath_len);
				if(ptr->newpath)
					DPRINTF("TO: %s (len: 0x%X byte(s))\n", ptr->newpath, (uint32_t)ptr->newpath_len);
				
				else{
					DPRINTF("TO: null mapped path (len: 0x%X byte(s))\n", (uint32_t)ptr->newpath_len);
				}
			}
			else{
				DPRINTF("FROM: null mapped path (len: 0x%X byte(s))\n", (uint32_t)ptr->oldpath_len);
			}
		}
}
#endif
		...
		if((strncmp(opath,"/dev_",5) == 0 || strncmp(opath,"/app_",5) == 0 || strncmp(opath,"/host_",6) == 0)) {
			...
			if(curr!=NULL) {
				...
				if(mapTableByteSize + sz <= MAX_TABLE_SIZE) {
					...
					#ifdef DEBUG
						DPRINTF("addMapping: Mapping Added: %s - Mappings Count: %u - Table Mapping Size: 0x%lx bytes\n", opath, getMapTableLength(), mapTableByteSize);
					#endif
					return 0;
				}
				else{
...
static MapEntry_t* getClosestMapping(const char *opath,uint32_t flags) {
	#ifdef DEBUG
		if(opath){
			DPRINTF("getClosestMapping: looking for mapping %s with flags 0x%X\n",opath,flags);
		}
	#endif
	MapEntry_t *ret = NULL;
	...
	size_t olen = 0;
	if(head && opath) {
		if(strlen(opath) > 6) {// length of /dev_cf == minimum mount point length
			//start from the first link, slot order matters
			for(curr = head; curr != NULL; curr = curr->next) {
				#ifdef DEBUG
					//DPRINTF("getClosestMapping: iterating through mapping %s to %s\n",curr->oldpath,curr->newpath);
				#endif
				if(flags>0 && curr->flags&flags && strncmp(curr->oldpath, opath, curr->oldpath_len) == 0){
					//if data found, return the current Link
					if(curr->oldpath_len>olen){
						olen = curr->oldpath_len;
						ret = curr;
						#ifdef DEBUG
							if(ret){
								DPRINTF("getClosestMapping: found mapping FROM: %s TO: %s with flags 0x%X\n",opath,ret->newpath,flags);
							}
						#endif
					}
				}
				else{
					if(strncmp(curr->oldpath, opath, curr->oldpath_len) == 0) {
						//if data found, return the current Link
						if(curr->oldpath_len>olen){
							olen = curr->oldpath_len;
							ret = curr;
							#ifdef DEBUG
								if(ret){
									DPRINTF("getClosestMapping: found mapping FROM: %s TO: %s without flags\n",opath,ret->newpath);
								}
							#endif
						}
					}
	...
	#ifdef DEBUG
		if(ret){
			//DPRINTF("getClosestMapping: found mapping FROM: %s TO: %s\n",opath,ret->newpath);
		}
		else {
			DPRINTF("getClosestMapping: found no mapping for %s\n",opath);
		}
	#endif
	return ret;
}
...
static MapEntry_t* findMapping(const char *opath) {
	#ifdef DEBUG
		if(opath){
			DPRINTF("findMapping: %s\n", opath);
		}
		else{
			DPRINTF("findMapping: null argument\n");
		}
	#endif
	MapEntry_t *ret = NULL;
	MapEntry_t *curr = NULL;
	...
	if(head && opath) {
		...
		if(len>6) { // length of /dev_cf == minimum mount point length
			mcount_found = false;
			curr = getClosestMapping(opath,FLAG_MAX_PRIORITY);
			...
			if(!ret) { // if no item with
				mcount_found = false;
				curr = getClosestMapping(opath,0);
				for(size_t j = 0; j < count; j++) {
					if(mcount[j] == curr) {
						mcount_found = true;
						// mapping found in mcount
		...
		if(ret) {
			...
			if(tmp_mapping) {
				...
				if(found!=ret) {
					found = ret;
					#ifdef DEBUG
						DPRINTF("findMapping: recursive call %s\n", tmp_mapping);
					#endif
	...
	#ifdef DEBUG
		if(ret){
			DPRINTF("findMapping: return %s\n", ret->newpath);
		}
		else{
			DPRINTF("findMapping: return null\n");
		}
	#endif
	return ret;
}
...
int map_path(char *oldpath, char *newpath, uint32_t flags)
{
	...
	if (oldpath) {
		...
		#ifdef DEBUG
			if(newpath)
				DPRINTF("map_path: attempting to map path: %s -> %s flags %x\n", oldpath, newpath, flags);
			else
				DPRINTF("map_path: attempting to remove map path: %s flags %x\n", oldpath, flags);
		#endif
		lock_mtx(&map_mtx);
		...
		unlock_mtx(&map_mtx);
			...
			if(ret==0) {
				if(newpath)
					...
					DPRINTF("map_path: removed map path: %s flags %x\n", oldpath, flags);
			}
			else{
				if(newpath)
					DPRINTF("map_path: add mapping error 0x%X for path: %s -> %s flags %x\n",ret, oldpath, newpath, flags);
				else
					DPRINTF("map_path: remove map error 0x%X for path: %s flags %x\n", ret, oldpath, flags);
			}
		#endif
	}
	return ret;
	...
	else
	{
		...
		for(unsigned int i = 0; i < num; i++)
		{
			...
			ret = map_path_user((char *)(uint64_t)u_paths[i], (char *)(uint64_t)u_new_paths[i], FLAG_TABLE);
				...
				if(ret==0) {
					if(u_new_paths[i]){
						DPRINTF("sys_map_paths: mapped path: %s -> %s flags %x\n", (char *)(uint64_t)u_paths[i], (char *)(uint64_t)u_new_paths[i], FLAG_TABLE);
					}
					else{
						DPRINTF("sys_map_paths: removed map path: %s flags %x\n", (char *)(uint64_t)u_paths[i], FLAG_TABLE);
					}
				}
				else{
					if(u_new_paths[i]){
						DPRINTF("sys_map_paths: add mapping error 0x%X for path: %s -> %s flags %x\n",ret, (char *)(uint64_t)u_paths[i], (char *)(uint64_t)u_new_paths[i], FLAG_TABLE);
					}
					else{
						DPRINTF("sys_map_paths: remove map error 0x%X for path: %s flags %x\n", ret, (char *)(uint64_t)u_paths[i], FLAG_TABLE);
					}
				}
			#endif
			if (ret != 0)
		...
		CellFsStat stat;
			...
			if(lretin != 0) {
				...
				if(cellFsStat(path0,&stat)) {
					DPRINTF("open_path_hook=: stat [NG] %s\n",path0);
				}
				else{
					DPRINTF("open_path_hook=: stat [OK] %s\n",path0);
				}
			}
		#endif
